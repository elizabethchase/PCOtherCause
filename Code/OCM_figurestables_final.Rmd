...
table_final$ContTable$NHANES["permth_exm", "median"] <- nhanes_medfu
table_final$ContTable$PLCO["permth_exm", "median"] <- plco_medfu

kable(print(table_final, contDigits=1, varLabel = T, printToggle=FALSE, minMax=TRUE, nonnormal = c("permth_exm")), booktabs=T) %>% add_indent(c(4:6, 8:12, 14:16, 18:20, 30:33))
...
Figure 1: Forest plot of predictors for final OC mortality prediction model fit in the NHANES training cohort of 7,369 men. The model also includes interactions between age and diabetes, education, hypertension, and stroke.
```{r, fig.align="center", fig.height=4, fig.width=6, message=FALSE, warning=FALSE, echo=FALSE}
cox_40 <-
  coxph(
...
Figure 2: Externally validated time-dependent AUCs at 5, 10, and 14 years of our Cox model, the Social Security Administration (SSA) 2001 actuarial life table predictions, and the National Vital Statistics System's (NVSS) 2001 life expectancy predictions. Models were validated in the Prostate, Lung, Colon, and Ovarian Cancer Screening Trial (PLCO) cohort of 8,220 men.
```{r, fig.align="center", fig.height=4, fig.width=6, message=FALSE, warning=FALSE, echo=FALSE, cache = TRUE}
cox_age <-
  coxph(
...
aucplot <- ggplot(data = subperf[subperf$Metric=="CS",], aes (x = Year, y = AUC, group = Model, color = Model)) + geom_point() + geom_line() + geom_errorbar(aes(ymin = Lower/100, ymax = Upper/100, width = 0.05)) + ylab("IPCW Time-Dependent AUC") + xlab("Time (Years)") + scale_x_continuous(breaks = c(0, 5, 10, 15)) + theme_bw() + scale_color_manual(values = c("darkred", "darkblue", "gray74", "mediumpurple4"))
...
Figure 3: Calibration performance of our final OC mortality model in PLCO validation cohort of 8,220 men with prostate cancer. 
```{r, fig.align="center", fig.height=4, fig.width=6, message=FALSE, warning=FALSE, echo=FALSE}
...
myplot <- ggplot() + geom_line(data = calibration_data, aes(x=Time, y=Observed, 
  ...
  geom_abline(slope = 1, intercept = 0) + scale_color_manual(values = c("mediumpurple4","gray74", "darkblue"), name = "Model") + theme(legend.position = "bottom") + coord_cartesian(ylim = c(2, 23), xlim = c(2,16))
...
Table 2: Percent of men with median survival 0-5, 5-10, 10-15, and 15+ years, grouped by age, in a cohort of 8,220 men with prostate cancer from the Prostate, Lung, Colorectal, and Ovarian Cancer Screening Trial (PLCO). Although older men generally have reduced life expectancy, we see that a substantial proportion of older men also have long life expectancy, which cannot be captured by predictions that rely on age alone.
```{r, fig.align="center", fig.height=4, fig.width=6, message=FALSE, warning=FALSE, echo=FALSE}
...
kable(lifeexpdat, col.names = c("Age", "0-5 Years", "5-10 Years", "10-15 Years", "15+ Years")) %>% column_spec(column = 1, width = "2cm") %>% column_spec(column = 2:5, width = "2cm")
...
\newpage
Figure 4: Proportion of men receiving a particular treatment in a cohort of 7,596 men in the Prostate, Lung, Colon, and Ovarian Cancer Screening Trial (PLCO), grouped by NCCN prostate cancer risk group and median survival prediction.
```{r, fig.align="center", fig.height=4, fig.width=6, message=FALSE, warning=FALSE, echo=FALSE}
...
prop_dataset$treatment <- factor(prop_dataset$treatment, levels=c("Active surveillance", "Radiation alone", "Radiation + hormone", "Prostatectomy"), ordered=TRUE)
